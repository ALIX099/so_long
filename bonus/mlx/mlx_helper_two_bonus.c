...
/*   Updated: 2025/04/05 12:03:17 by abouknan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../so_long_bonus.h"

void	reach_empty_or_coin(t_game *game, int new_x, int new_y, char *move_name)
{
	if (game->map[new_y][new_x] == 'C' || game->map[new_y][new_x] == '0')
	{
		...
		if (!ft_strcmp(move_name, "UP"))
			mlx_put_image_to_window(game->mlx_init, game->mlx_win,
				game->player_up, game->player_x * 40, game->player_y * 40);
		else if (!ft_strcmp(move_name, "LEFT"))
			mlx_put_image_to_window(game->mlx_init, game->mlx_win,
				game->player_left, game->player_x * 40, game->player_y * 40);
		else if (!ft_strcmp(move_name, "DOWN"))
			mlx_put_image_to_window(game->mlx_init, game->mlx_win,
				game->player_down, game->player_x * 40, game->player_y * 40);
		else if (!ft_strcmp(move_name, "RIGHT"))
			mlx_put_image_to_window(game->mlx_init, game->mlx_win,
				game->player_right, game->player_x * 40, game->player_y * 40);
